# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 17:02:38 2019
...
class Anomalydetection(object):
    ...
    def Train(self, X_train):
        ...
        '''Sigma表示每个特征的标准差,为n维向量,n为数据特征数量'''
    ...
    def Detection(self, X):
        ...
        for i in range(X.shape[0]):
            for j in range(X.shape[1]):
                p[i] *= 1./(math.sqrt(2*math.pi)*self.Sigma[j]) * math.exp(-pow((X[i][j]-self.Mu[j]),2)/(2*pow(self.Sigma[j],2)))
            if p[i] < self.Epsilon:
                result[i] = 1
            else:
                result[i] = 0
